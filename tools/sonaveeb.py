...
def example(result, head, limit=70, ru_hint="", sense=None):
    ...
    Пример берём только из значений, совпавших с нашим переводом (или из
    значения eki_sense), в порядке статьи: первое, где есть годный пример.
    ...
    Рекция (rection) берётся строго из ПЕРВОГО совпавшего значения. Поэтому,
    если в нём нет годного примера, пример и рекция окажутся из разных
    значений — оба совпадают с переводом, но это не одно и то же значение.
    """
    meanings = result.get("meanings") or []
    if not meanings:
        return ""

    # тот же список совпавших значений, что и для рекции (рекция — только из первого);
    # перевод не опознан — главное значение
...
def ru_check():
    ...
    for kind, key in (("noun", "nouns"), ("verb", "verbs")):
        for w in d[key]:
            ...
            if failed_request(data):
                continue
            # статья выбирается по переводу, как в fix и check: иначе при омонимах
            # сверяем перевод с чужим значением (kiilakas «лысый» против «удар»).
            # add отличается только запасным путём: если перевод не совпал ни с одной
            # статьёй, он предпочитает статью-глагол, а fix/check — статью своего типа
            res = pick_best(data, kind, w.get("ru", ""))
            if not res:
                continue
